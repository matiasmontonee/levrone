    ...
    <section id="contacto">
      <main className={`${isScrolled ? 'lg:mt-20 mt-16' : ''}`}>
        <div className='p-24 bg-red-500'>
          <h2 className='sm:text-4xl text-2xl text-white text-center font-bold'>Contacto</h2>
        </div>
        ...
        <div className='flex flex-col lg:flex-row mt-8 mb-8'>
          ...
          <div className='w-full lg:w-1/2 flex items-center'>
            <form className="w-full p-8 mx-auto" onSubmit={handleSubmit} noValidate>
              <div className="mb-4">
                <label htmlFor="name" className="block text-gray-800 text-md sm:text-lg font-bold mb-2">Nombre</label>
                <input type="text" id="name" name="name" value={name} onChange={(e) => setName(e.target.value)} placeholder='Ingrese su nombre' className="appearance-none bg-gray-200 border border-gray-200 rounded-lg w-full p-2 focus:outline-none focus:border-red-500" style={{ borderWidth: '3px' }} />
              ...
              <div className="mb-4">
                ...
                <input type="email" id="email" name="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder='Ingrese su correo' className="appearance-none bg-gray-200 border border-gray-200 rounded-lg w-full p-2 focus:outline-none focus:border-red-500" style={{ borderWidth: '3px' }} />
                {emailError && <p className="text-red-500 mt-1">{emailError}</p>}
              </div>
              <div className="mb-4">
                <label htmlFor="comment" className="block text-gray-800 text-md sm:text-lg font-bold mb-2">Comentario</label>
                <textarea id="comment" name="comment" value={comment} onChange={(e) => setComment(e.target.value)} placeholder='Quería contactarte para...' className="appearance-none bg-gray-200 border border-gray-200 rounded-lg w-full p-6 pt-1 pl-2 focus:outline-none focus:border-red-500" style={{ borderWidth: '3px' }}></textarea>
                {commentError && <p className="text-red-500">{commentError}</p>}
              </div>
              <div className="mb-4">
                <label htmlFor="userType" className="block text-gray-800 text-md sm:text-lg font-bold mb-2">Tipo de Usuario</label>
                <select id="userType" name="userType" value={userType} onChange={(e) => setUserType(e.target.value)} className="appearance-none bg-gray-200 border border-gray-200 rounded-lg w-full p-2 focus:outline-none focus:border-red-500" style={{ borderWidth: '3px' }}>
              ...
              {userType === 'profesor' && ( 
                <div className="mb-4" onDrop={handleDrop}>
                  ...
                  {(!certificateFile && userType === 'profesor') && <p className="text-red-500 mt-1">Por favor, cargue su certificado de profesor</p>}
                  {fileTypeError && <p className="text-red-500 mt-1">{fileTypeError}</p>}
                </div>
              )}
              <div className="flex items-center justify-end">
                <button type="submit" className="bg-red-600 hover:bg-red-400 rounded-full text-gray-200 font-bold py-2 text-center inline-block w-28">Enviar</button>
              </div>
            </form>
          </div>
        </div>

        {showSuccessModal && <ModalExito onClose={() => setShowSuccessModal(false)} />}
        {showErrorModal && <ModalError onClose={() => setShowErrorModal(false)} />}
      </main>
    </section>
  );
};
