...
const Confirmacion = () => {
  ...
  if (noCompras) {
    return <div className="text-center my-8">No has realizado ninguna compra recientemente.</div>;
  }
      ...
      <main className={`${isScrolled ? 'lg:mt-20 mt-16' : ''}`}>
        <div className='mx-auto p-8 w-3/4'>
          <div className="text-center">
            <h1 className="text-3xl font-bold text-green-500">¡Compra confirmada!</h1>
            <p className="my-2 text-gray-700">Gracias por tu compra. Tu pedido ha sido confirmado.</p>
            <p className='text-xl font-bold'>Total: ${compra.precioTotalCarrito}</p>
          </div>
            ...
            {compra.carrito.map((producto, index) => (
              <div key={index} className="flex flex-col md:flex-row items-center border-b-2">
                ...
                <div className="md:ml-8 mt-4 md:mt-0">
                  ...
                  <p className="md:mb-0 mb-4 font-bold">${producto.precioTotal}</p>
                </div>
              </div>
            ))}
          </div>
          <div className="flex flex-col md:flex-row justify-center items-center md:space-x-4">
            <Link to={'/productos'}><button className="bg-green-500 text-white px-4 py-2 rounded-md hover:bg-green-400 mb-4 md:mb-0">Continuar Comprando</button></Link>
            <Link to={'/compras'}><button className="bg-gray-400 text-white px-4 py-2 rounded-md hover:bg-gray-500">Ver Pedido</button></Link>
          </div>
        </div>
      </main>
    </section>
