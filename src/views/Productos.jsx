    ...
    <section id='productos'>
      ...
      <main className={`${isScrolled ? 'lg:mt-20 mt-16' : ''}`}>
        <div className="flex flex-col p-8 pt-6 pb-0 relative">
          ...
          <span className="absolute inset-y-0 right-12 bottom-10 flex items-center">
            <FaSearch />
          </span>
                ...
                {producto.descuento ? (
                  <>
                    <div className='flex items-center'>
                      <p className="text-xl font-bold mr-2">${producto.descuento}</p>
                      <p className="text-gray-500 line-through">${producto.precio}</p>
                    </div>
                    <button onClick={() => agregarAlCarrito(producto)} className='text-2xl text-red-500 hover:text-red-400'><FaShoppingCart /></button>
                  </>
                ) : (
                  <>
