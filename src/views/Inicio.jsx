...
const Inicio = () => {
  ...
  return (
    <section id='inicio'>
      <main className={`bg-gray-100 ${isScrolled ? 'lg:mt-20 mt-16' : ''}`}>
            ...
            <div className="flex transition-transform duration-500 ease-in-out" style={{ transform: `translateX(-${currentImageIndex * 100}%)` }}>
              {images.map((image, index) => (
                <div key={index} className="relative flex-shrink-0 w-full">
                  ...
                  {index === 3 && (
                    <button className="btn-banner absolute bottom-0 top-0 left-0 right-0 m-auto h-7 sm:h-10 w-32 sm:w-40 text-gray-100 bg-blue-500 hover:bg-blue-400 py-1 sm:py-2 px-2 rounded-full focus:outline-none z-10 font-bold">Ver suplementos</button>
                  )}
                  {index === 4 && (
                    <button className="btn-banner absolute bottom-0 top-0 left-0 right-0 m-auto h-7 sm:h-10 w-32 sm:w-40 text-white bg-red-600 hover:bg-red-400 py-1 sm:py-2 px-2 rounded-full focus:outline-none z-10 font-bold">Ver muñequeras</button>
                  )}
                </div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 mt-8">
          {productos.map(producto => (
            <div key={producto?.id} className="w-80 mx-auto mb-8">
              <div className="h-full border border-white hover:border-red-500 rounded-lg overflow-hidden bg-white flex flex-col justify-between">
                <Link to={`/productos/${producto?.id}`} className="flex flex-col h-full">
                  ...
                  <div className="p-4 flex flex-col justify-between flex-grow">
                    <p className="text-sm text-red-500 font-semibold">{producto?.marca}</p>
                    <div className='flex flex-col justify-between flex-grow'>
                      <p className="font-semibold">{producto?.nombre}</p>
                      <p className='text-gray-500 line-through mt-4'>${producto?.precio}</p>
                    </div>
                    <div className='flex justify-between'>
                      <p className="text-xl font-semibold">${producto?.descuento}</p>
                      <FaShoppingCart className='text-2xl text-red-500' />
                    </div>
                  </div>
                </Link>
                <Link to={`/productos/${producto?.id}`}>
                  <button className='w-full bg-red-500 p-2 text-white hover:bg-red-400 font-semibold'>Ver detalle</button>
                </Link>
              </div>
            </div>
          ))}
        ...
        <div className='flex justify-center mb-8'>
          <Link to={"/productos"} className="border font-bold text-gray-100 bg-red-600 hover:bg-red-400 md:px-8 px-4 md:py-3 py-2 rounded-full">Ver todos los productos</Link>
        </div>
