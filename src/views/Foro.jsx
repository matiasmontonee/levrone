    ...
    <section id='foro'>
      <main>
        ...
        <div className='flex justify-center mt-4'>
          <Link to='/post'>
            <button className='rounded-md bg-red-500 p-2 text-white hover:bg-red-400 font-semibold'>Agregar un post</button>
          </Link>
        </div>
            ...
            {posts.map(post => (
              <div key={post.id} className='border border-gray-400 rounded p-4 my-4'>
                ...
                <div className='flex justify-between items-center mt-2'>
                  <button className='flex items-center rounded-md bg-blue-500 p-1 px-2 text-white hover:bg-blue-400 font-semibold' onClick={() => handleCommentClick(post.id)}>Comentar   <FaComment className='ml-1' /></button>
                  <p className='text-gray-500'>{new Date(post.createdAt.seconds * 1000).toLocaleString()}</p>
                </div>
                {commentVisible[post.id] && (
                  <div className='mt-2'>
                    <textarea className='w-full p-2 border border-gray-400 rounded' rows='3' value={comment} onChange={(e) => setComment(e.target.value)} placeholder='Escribe tu comentario...'></textarea>
